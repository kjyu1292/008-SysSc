...
extern int32_t _sumBetween(int32_t* d, int32_t n, int32_t i, int32_t j);
...
custom_arr* solve(int32_t* D, int32_t num_elems, int32_t o, int32_t h) {
	...
	int32_t F[num_elems];
	int32_t cover_by[num_elems];
	int32_t min_args[num_elems];
	...
	for (i = 0; i < num_elems; i++) {
		...
		for (j = i; j >= t_star_star; j--) {
			S_t += h * _sumBetween(D, num_elems, j+1, i);
			dummy = S_t + o + F[j];
